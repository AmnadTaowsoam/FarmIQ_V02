  ...
  try {
    ...
    if (process.env.RABBITMQ_URL || process.env.NODE_ENV !== 'test') {
      try {
        ...
        logger.info('RabbitMQ connection established for event publishing')
      } catch (error) {
        logger.warn('Failed to connect to RabbitMQ (non-fatal, service will continue)', {
          error,
    ...
    server = app.listen(port, () => {
      logger.info(`cloud-barn-records-service running on port ${port}`)
