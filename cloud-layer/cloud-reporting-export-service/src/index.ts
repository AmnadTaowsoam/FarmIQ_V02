...
import { prismaConnect, prismaDisconnect, getPrismaClient } from './services/reportJobService'
import { connectRabbitMQ, closeRabbitMQ, isRabbitConnected } from './utils/rabbitmq'
import { startReportJobConsumer } from './services/reportWorker'
...
async function startServer() {
  try {
    ...
    await startReportJobConsumer()

    server = app.listen(port, () => {
      logger.info(`cloud-reporting-export-service running on port ${port}`)
    })
