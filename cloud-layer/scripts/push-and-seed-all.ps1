...
# List of services with their database names
$services = @(
    @{ name = "cloud-identity-access"; hasSeed = $true },
    @{ name = "cloud-tenant-registry"; hasSeed = $true; priority = 1 },
    @{ name = "cloud-ingestion"; hasSeed = $false },
    @{ name = "cloud-telemetry-service"; hasSeed = $true },
    @{ name = "cloud-api-gateway-bff"; hasSeed = $false },
    @{ name = "cloud-config-rules-service"; hasSeed = $true },
    @{ name = "cloud-audit-log-service"; hasSeed = $true },
    @{ name = "cloud-notification-service"; hasSeed = $false },
    @{ name = "cloud-reporting-export-service"; hasSeed = $true },
    @{ name = "cloud-feed-service"; hasSeed = $true },
    @{ name = "cloud-barn-records-service"; hasSeed = $true },
    @{ name = "cloud-weighvision-readmodel"; hasSeed = $false }
)

# Sort services by priority (tenant-registry first)
$sortedServices = $services | Sort-Object { if ($_.priority) { $_.priority } else { 999 } }
...
if (-not $SkipPush) {
    ...
    foreach ($service in $sortedServices) {
        ...
        $result = docker compose -f $DockerComposeFile exec -T $serviceName npx prisma db push --accept-data-loss --skip-generate 2>&1
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            docker compose -f $DockerComposeFile exec -T $serviceName npx prisma generate 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) {
...
if (-not $SkipSeed) {
    ...
    foreach ($service in $sortedServices) {
        ...
        foreach ($cmd in $seedCommands) {
            $result = docker compose -f $DockerComposeFile exec -T $serviceName $cmd 2>&1
